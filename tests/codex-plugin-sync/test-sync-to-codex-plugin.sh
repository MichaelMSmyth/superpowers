...
EOF
    ...
    cat > "$repo/.opencode/plugin.json" <<EOF
{
  "name": "superpowers",
  "version": "$MANIFEST_VERSION"
}
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/superpowers/.opencode" \
        "$repo/plugins/superpowers/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/superpowers/.fixture-keep"
    printf '{"name":"stale-opencode"}\n' > "$repo/plugins/superpowers/.opencode/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/superpowers/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/superpowers/.fixture-keep \
        plugins/superpowers/.opencode/plugin.json
...
main() {
    ...
    assert_not_contains "$preview_section" ".opencode/plugin.json" "Preview excludes foreign-harness dotdir from Codex sync"
    ...
    assert_matches "$stale_preview_section" "\\*deleting +\\.opencode/plugin\\.json" "Preview deletes stale excluded manifest from Codex plugin"
